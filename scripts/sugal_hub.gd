...
func setup_game() -> void:
	minigame_id = "sugal"
	reward_amount = 0 # Handled manually during spins
...
func spin() -> void:
	...
	if GameState.sugal_visits == 1 and spins_this_session == 1:
		rig_state = "HONEYMOON"
	elif RunState.money <= current_bet and RunState.money > 0:
		rig_state = "MERCY"
	else:
		rig_state = "NORMAL"
...
func update_ui() -> void:
	balance_label.text = "E-Pera: ₱" + str(RunState.money)
...
func execute_exit() -> void:
	...
	if RunState.gambling_attempted:
		var session_profit = RunState.money - starting_money
		RunState.gambling_net_result += session_profit
		print("Gambling locked. Net profit/loss: ", session_profit)
